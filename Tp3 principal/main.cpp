...
bool
validerNom (const string& nom)
{
  return validerFormatNom (nom);
}

bool
validerTelephone (const string& tel)
{
  return !tel.empty ();
}
...
main ()
{
  ...
  do
    {
      cout << "numéro de compte (entier positif) : ";
      cin >> noCheque;
    }
  while (noCheque <= 0);
  ...
  do
    {
      cout << "Description (non vide) : ";
      getline (cin, descCheque);
    }
  while (descCheque.empty ());

  do
    {
      cout << "Taux d'Intéret (supérieur au taux minimum) : ";
      cin >> tauxCheque;
    }
  while (cin.fail ());
  ...
  do
    {
      cout << "Nombre de transaction (entre 0 et 40) : ";
      cin >> nbTrans;
    }
  ...
  do
    {
      cout << "Taux d'intérêt minimum (inférieur au taux d'intérêt) : ";
      cin >> tauxMinCheque;
    }
